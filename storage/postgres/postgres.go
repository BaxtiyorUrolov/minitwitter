...
func New(ctx context.Context, cfg config.Config, log logger.Logger) (storage.IStorage, error) {
	url := fmt.Sprintf(
...
func (s Store) Tweet() storage.ITweetStorage {
	return NewTweetRepo(s.pool, s.log)
}

func (s Store) Like() storage.ILikeStorage {
	return NewLikeRepo(s.pool, s.log)
}

func (s Store) Follow() storage.IFollowStorage {
	return NewFollowRepo(s.pool, s.log)
}
